...
def assert_shape(actual, expected, key=None):
    ...
    if key is not None:
        suffix = " for parent " + ("index" if isinstance(key, int) else "key") + f" {key!r}"
    ...
    if isinstance(expected, Mapping):
        ...
        for key, value in expected.items():
            if value is not UNDEFINED:
                ...
                assert key not in actual, f"expected key {key!r} not found in: {actual!r}" + suffix
        ...
        assert abs(actual - expected) < 1e-6, "float not within the allowed tolerance of 1e-6"
    else:
